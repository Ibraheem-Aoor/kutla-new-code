...
/** STATUS CHANGE FOR TOGGLE BUTTONS */
$('.toggle-status').on('change', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if(status === 1){
                toastr.success(response.message + ' menu published');
            } else {
                toastr.success(response.message + ' menu unpublished');
            }
        },
        error: function(xhr, status, error) {
            console.log(error);
...
/** STATUS CHANGE FOR IS ACTIVE BUTTONS */
$('.toggle-is-active').on('change', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if(status === 1){
                toastr.success(response.message + ' IsActive published');
            } else {
                toastr.success(response.message + ' IsActive unpublished');
            }
        },
        error: function(xhr, status, error) {
            console.log(error);
...
/** STATUS CHANGE FOR BREAKING NEWS BUTTONS */
$('.toggle-breaking').on('change', function() {
    var checkbox = $(this);
    var status = checkbox.prop('checked') ? 1 : 0;
    var url = checkbox.data('url');
