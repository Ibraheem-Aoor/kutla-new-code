...
@section('main_content')
    ...
    <nav aria-label="breadcrumb" class="maan-breadcrumb">
        <div class="container">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ URL('/') }}">{{ get_option('manage-pages')['headings']['home_title'] ?? '' }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ $newscategorysingle->name ?? 'No Post' }}</li>
            </ol>
        </div>
    </nav>
    <!-- Maan Breadcrumb End -->
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                    <div class="maan-title">
                        <div class="maan-title-text">
                            <h2>{{ $newscategorysingle->name ?? 'No Post' }}</h2>
                        </div>
                    </div>
                    <div class="maan-news-post">
                            ...
                            @foreach($allnews as $allnewsvalue )
                                <div class="card maan-default-post">
                                            ...
                                            @if($images!='')
                                                @foreach ($images as $image)
                                                    @if (File::exists($image))
                                                        <a href="{{ route('news.details', ['category_name' => $allnewsvalue->news_categoryslug, 'id' => $allnewsvalue->id, 'slug' => Str::slug($allnewsvalue->title)]) }}">
                                    ...
                                    <div class="card-body maan-card-body">
                                        <div class="maan-text">
                                            <h4><a href="{{ route('news.details', ['category_name' => $allnewsvalue->news_categoryslug, 'id' => $allnewsvalue->id, 'slug' => Str::slug($allnewsvalue->title)]) }}">{{ $allnewsvalue->title }}</a></h4>
                                            <ul>
                                                <li>
                        ...
                        @foreach($allnews as $allnewsall )
                            <div class="card maan-default-post">
                                        ...
                                        @if($images!='')
                                            @foreach ($images as $image)
                                                @if (File::exists($image))
                                                    ...
                                                    <a href="{{ route('news.details', ['category_name' => $allnewsall->news_categoryslug, 'id' => $allnewsall->id, 'slug' => Str::slug($allnewsall->title)]) }}">
                                                        <img loading="lazy" src="{{ asset($image) }}" alt="top-news">
                                                    </a>
                                                @endif
                                            @endforeach
                                ...
                                <div class="card-body maan-card-body">
                                    <div class="maan-text">
                                        <h4><a href="{{ route('news.details', ['category_name' => $allnewsall->news_categoryslug, 'id' => $allnewsall->id, 'slug' => Str::slug($allnewsall->title)]) }}">{{ $allnewsall->title }}</a></h4>
                ...
                <div class="col-lg-4">
                    <div class="maan-title">
                        <div class="maan-title-text">
                            <h2>{{ get_option('manage-pages')['headings']['popular_post_title'] ?? '' }}</h2>
                        </div>
                    </div>
                        ...
                        <div class="popular-post">
                            @foreach($popularallnews as $popularnews)
                                <div class="card maan-default-post">
                                    <div class="maan-post-img">
                                        @if ($popularnews->image)
                                            ...
                                            @if($images!='')
                                                @foreach ($images as $image)
                                                    @if (File::exists($image))
                                                        <a href="{{ route('news.details', ['category_name' => $popularnews->news_categoryslug, 'id' => $popularnews->id, 'slug' => Str::slug($popularnews->title)]) }}">
                                                            <img loading="lazy" src="{{ asset($image) }}" alt="top-news">
                                                        </a>
                                                    @endif
                                                @endforeach
                                    ...
                                    <div class="card-body maan-card-body">
                                        <div class="maan-text">
                                            <h4><a href="{{ route('news.details', ['category_name' => $popularnews->news_categoryslug, 'id' => $popularnews->id, 'slug' => Str::slug($popularnews->title)]) }}">{{ $popularnews->title }}</a></h4>
                                            <ul>
                                                <li>
                                                    <span class="maan-icon"><svg viewBox="0 0 512 512"><path d="M347.216,301.211l-71.387-53.54V138.609c0-10.966-8.864-19.83-19.83-19.83c-10.966,0-19.83,8.864-19.83,19.83v118.978 c0,6.246,2.935,12.136,7.932,15.864l79.318,59.489c3.569,2.677,7.734,3.966,11.878,3.966c6.048,0,11.997-2.717,15.884-7.952 C357.766,320.208,355.981,307.775,347.216,301.211z"></path><path d="M256,0C114.833,0,0,114.833,0,256s114.833,256,256,256s256-114.833,256-256S397.167,0,256,0z M256,472.341 c-119.275,0-216.341-97.066-216.341-216.341S136.725,39.659,256,39.659c119.295,0,216.341,97.066,216.341,216.341 S375.275,472.341,256,472.341z"></path></svg></span>
                    ...
                    <div class="maan-title">
                        <div class="maan-title-text">
                            {{-- <h2>{{ $page_data['headings']['gallery_title'] ?? '' }}</h2> --}}
                            <h2>{{ get_option('manage-pages')['headings']['gallery_title'] ?? '' }}</h2>
                        </div>
                    </div>
                    ...
                    <div class="maan-title">
                        <div class="maan-title-text">
                            {{-- <h2>{{ $page_data['headings']['recent_post_title'] ?? '' }}</h2> --}}
                            <h2>{{ get_option('manage-pages')['headings']['recent_post_title'] ?? '' }}</h2>
                        </div>
                    </div>
                    <div class="maan-widgets maan-bg-tr">
                                ...
                                @foreach($recentallnews as $recentallnews)
                                    <li>
                                            ...
                                            @if ($recentallnews->image)
                                                ...
                                                @if($images!='')
                                                    @foreach ($images as $image)
                                                        ...
                                                        @if (File::exists($image))
                                                            <a href="{{ route('news.details', ['category_name' => $recentallnews->news_categoryslug, 'id' => $recentallnews->id, 'slug' => Str::slug($recentallnews->title)]) }}">
                                        ...
                                        <div class="maan-list-text">
                                            <h4><a href="{{ route('news.details', ['category_name' => $recentallnews->news_categoryslug, 'id' => $recentallnews->id, 'slug' => Str::slug($recentallnews->title)]) }}">{{ $recentallnews->title }}</a></h4>
                                            <ul>
                                                <li>
                                                    <span class="maan-icon"><svg viewBox="0 0 512 512"><path d="M347.216,301.211l-71.387-53.54V138.609c0-10.966-8.864-19.83-19.83-19.83c-10.966,0-19.83,8.864-19.83,19.83v118.978 c0,6.246,2.935,12.136,7.932,15.864l79.318,59.489c3.569,2.677,7.734,3.966,11.878,3.966c6.048,0,11.997-2.717,15.884-7.952 C357.766,320.208,355.981,307.775,347.216,301.211z"></path><path d="M256,0C114.833,0,0,114.833,0,256s114.833,256,256,256s256-114.833,256-256S397.167,0,256,0z M256,472.341 c-119.275,0-216.341-97.066-216.341-216.341S136.725,39.659,256,39.659c119.295,0,216.341,97.066,216.341,216.341 S375.275,472.341,256,472.341z"></path></svg></span>
                    ...
                    <div class="maan-title">
                        <div class="maan-title-text">
                            <h2>{{ get_option('manage-pages')['headings']['tags_title'] ?? '' }}</h2>
                        </div>
                    </div>
                    <div class="maan-widgets">
