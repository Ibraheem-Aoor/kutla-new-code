        ...
        <div class="container">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a
                        href="{{ URL('/') }}">{{ get_option('manage-pages')['headings']['home_title'] ?? '' }}</a>
                </li>
                    ...
                    <div class="maan-news-post">
                            ...
                            @foreach ($allnews as $allnewsvalue)
                                <div class="card maan-default-post">
                                            ...
                                            @if ($images != '')
                                                @foreach ($images as $image)
                                                    @if (File::exists($image))
                                                        <a href="{{ return_post_link($allnewsvalue) }}">
                                    ...
                                    <div class="card-body maan-card-body">
                                        <div class="maan-text">
                                            <h4><a
                                                    href="{{ return_post_link($allnewsvalue) }}">{{ $allnewsvalue->title }}</a>
                                            </h4>
                                            <ul>
                                                <li>
                                                ...
                                                <li>
                                                    ...
                                                    <span
                                                        class="maan-item-text">{{ date('d M, Y', strtotime($allnewsvalue->date)) }}</span>
                        ...
                        @foreach ($allnews as $allnewsall)
                            <div class="card maan-default-post">
                                <div class="maan-post-img">
                                    @if ($allnewsall->image)
                                        ...
                                        @if ($images != '')
                                            @foreach ($images as $image)
                                                @if (File::exists($image))
                                                    <a href="{{ return_post_link($allnewsall) }}">
                                                        <img loading="lazy" src="{{ asset($image) }}" alt="top-news">
                                                    </a>
                                                @endif
                                            @endforeach
                                ...
                                <div class="card-body maan-card-body">
                                    <div class="maan-text">
                                        <h4><a href="{{ return_post_link($allnewsall) }}">{{ $allnewsall->title }}</a>
                                        </h4>
                            ...
                            @foreach ($popularallnews as $popularnews)
                                <div class="card maan-default-post">
                                    <div class="maan-post-img">
                                        @if ($popularnews->image)
                                            ...
                                            @if ($images != '')
                                                @foreach ($images as $image)
                                                    @if (File::exists($image))
                                                        <a href="{{ return_post_link($popularnews) }}">
                                    ...
                                    <div class="card-body maan-card-body">
                                        <div class="maan-text">
                                            <h4><a
                                                    href="{{ return_post_link($popularnews) }}">{{ $popularnews->title }}</a>
                                            </h4>
                                            <ul>
                                                <li>
                                                    <span class="maan-icon"><svg viewBox="0 0 512 512">
                                ...
                                @foreach ($recentallnews as $recentallnews)
                                    <li>
                                        <div class="maan-list-img">
                                            @if ($recentallnews->image)
                                                ...
                                                @if ($images != '')
                                                    @foreach ($images as $image)
                                                        @if (File::exists($image))
                                                            <a href="{{ return_post_link($recentallnews) }}">
                                                                <img loading="lazy" src="{{ asset($image) }}"
                                                                    alt="list-news-img">
                                                            </a>
                                                        @endif
                                        ...
                                        <div class="maan-list-text">
                                            <h4><a
                                                    href="{{ return_post_link($recentallnews) }}">{{ $recentallnews->title }}</a>
                                            </h4>
                                            <ul>
                                                <li>
                                                    <span class="maan-icon"><svg viewBox="0 0 512 512">
                        ...
                        <div class="widgets-tags">
                            <ul>
                                @foreach (newscategories() as $newscategory)
                                    <li><a
                                            href="{{ route('categories.item', ['category_slug' => $newscategory->slug, 'category_name' => $newscategory->name]) }}">{{ $newscategory->name }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
